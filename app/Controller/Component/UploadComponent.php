    ...
    function image($file, $type, $size, $output = NULL, $quality = NULL) {
        ...
        switch ($type) {
			...
			case 'resize_img':
				...
				{
					$width=$size;
					$height = ($maxW/$uploadWidth)*$uploadHeight;
				}
				else
				{
					$width=$size;
				...
				$img_des = ImageCreateTrueColor($maxW,$maxH);
				imagecopyresampled ($dstImg, $srcImg, 0, 0, 0, 0, $maxW, $maxH, $uploadWidth, $uploadHeight);*/
				  // -- a straight centered crop
              // -- resize to max, then crop to center
                $ratioX = $maxW / $uploadWidth;
                $ratioY = $maxH / $uploadHeight;
				$origWidth = $uploadWidth;	
				$origHeight = $uploadHeight;	
                if ($ratioX < $ratioY) {
                    $newX = round(($uploadWidth - ($maxW / $ratioY)) / 2);
                    $newY = 0;
                    $uploadWidth = round($maxW / $ratioY);
				...
				if($origWidth >= $maxW ){
					...
					if($origHeight > $maxH){
						 ...
						 # within the maxW and maxH (thus some side will be smaller)						
						$newX = $origWidth -116;
						$newY = ($origHeight * $newX) / $origWidth;						
						$dstImg = imagecreatetruecolor($newX, $newY);
						imagecopyresampled($dstImg, $srcImg, 0, 0, 0, 0, $newX, $newY, $origWidth, $origHeight);						
					}
					else{
					   	
						$dstImg = imagecreatetruecolor($origWidth, $origHeight);
						$white = imagecolorallocate($dstImg, 255, 255, 255);
						imagefill($dstImg, 0, 0, $white);
						imagecolortransparent($dstImg,$white);
						imagecopyresampled($dstImg, $srcImg, 0, 0, 0, 0, $origWidth, $origHeight, $origWidth, $origHeight);
				     }	
				}
